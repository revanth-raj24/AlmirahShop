        ...
        {onToggleWishlist && (
          <div className="absolute top-4 right-4 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
            <button
              ...
              className="p-2 bg-white/90 hover:bg-white transition-colors duration-300 shadow-md"
            >
              <Heart
                className={`w-5 h-5 ${isInWishlist ? 'fill-neutral-900 text-neutral-900' : 'text-neutral-900'}`}
              />
            </button>
            <button
              onClick={handleAddToCart}
              disabled={isLoading}
              className="p-2 bg-white/90 hover:bg-white transition-colors duration-300 shadow-md disabled:opacity-50"
            >
